import sys
#sys.path.append('/home/macul/libraries/mk_utils/locallib/nn/')
import mxnet as mx
from locallib.nn.mxnet.mxmobilefacenet import MxMobileFaceNet
from locallib.nn.mxiter.mxiter import ImageMultiLabelIter, MyImageIter
from locallib.nn.mxloss.mxloss import MxLosses
import numpy as np
...
if config.Mobilefacenet_version=='v1':
	build_net = MxMobileFaceNet.build_v1
else:
	build_net = MxMobileFaceNet.build_v2

# if there is no specific model starting epoch supplied, then initialize the network
if args["start_epoch"] <= 0:
	# build the LeNet architecture
	print("[INFO] building network...")
	model, model_landmark = build_net(	data, config.Embedding_Size)
